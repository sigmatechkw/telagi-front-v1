...
const ExpertTransactionsListTableHeader = ({ selectedRows, onChange, value, clearSearch, fetchData, canExport }) => {
  ...
  const handleDelete = () => {
    deleteExpertTransactions(selectedRows).then(res => {
      setOpenDeleteSnackbar(false)
      fetchData()
    })
  }
  ...
  const handlePay = () => {
    payExpertTransactions(selectedRows).then(res => {
      setOpenPaySnackbar(false)
      fetchData()
    })
  }
      ...
      <Box sx={{rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center'}}>
        {
          ...
          <Box>
            <Button sx={{ marginInline: 2}} color={'success'} variant={'tonal'} onClick={handleClickPayButton}>
              <Icon fontSize='1.125rem' icon='tabler:brand-cashapp' />&nbsp;
              {t('pay_selected_rows')}
            </Button>
...
export default ExpertTransactionsListTableHeader
