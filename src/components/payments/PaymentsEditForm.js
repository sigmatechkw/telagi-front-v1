...
const PaymentsEditForm = ({
  ...
  const handleCredCheckBox = (e, val, id) => {
    setCredentials(prevData => prevData.map(item => (item.id === id ? { ...item, value: val } : item)))
  }
      ...
      <CardContent>
        <form onSubmit={onSubmit}>
          <Grid container spacing={4} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'between', gap: 4, alignItems: 'center' }}>
